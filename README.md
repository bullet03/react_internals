...
- У планировщика есть внешнее api => sheduleCallback, который берет функцию-callback по построению react дерева к себе в обработку (performConcurrentWorkOnRoot). Все дальнейшее просиходит в рамках этой функции
...
- Чтобы взять задачи из taskQueue надо запустить цикл обработки задач, который работет на уровне планировщика. Собственный цикл планировщика осуществляется через while. Данная функция с while называется workLoop. Схема такая flushwork => workLoop => while => task
