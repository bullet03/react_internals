...
3 аксиомы:
- новый react element -> новые props
- props от react element вливаются в fiber
- сравниваем state/props WIP Fiber с Current Fiber, чтобы разобраться с детьми => 
  изменение state/props WIP Fiber (на котором стоим) перевызовет функциональный компонент этого Fiber, который return новых детей в виде react element(s) => 
  эти react element(s) отправятся в function reconcile для создания дочерних WIP Fiber.
_______________________________________________________________________________________________________________
Hooks/Хуки
- Имеются разные определения. Hooks - команда, API. Команда - потому что отсутствует точка привязки жизненного цикла, что является критически важным в определении хука. Мы не против определения хука, потому что с точки зрения исходников там есть точка привязки жизненного цикла, но для пользователя она скрыта. API - можно называть хуки, это официальные команды по управлению fiber
- Hooks хранятся в fiber.memoizedState. У hook самого тоже есть своё поле memoizedStat, но назначение разное. hook.memoizedState сохраняет информацию между кадрами. Чтобы сохранить информацию между кадрами хуки клонируются. Оболочка объекта hook новая, а содержимое старое берется по ссылкам из currentHook
- Перед каждым вызовом функционального компонента fiber.memoizedState = null, список хуков пересоздаётся с нуля
- Глобально hooks делятся на 2 тиап: состояния (state) и эффектов (логики). State hooks тригерят обновление кадра (lanes, вызывая scheduleUpdateOnFiber), а Effect Hooks нет (меняют только flags)
- Структура hook (базово все хуки имеют эти 2 поля):
  - memoizedState (разное назначение в зависимости от типа hook)
  - next (ссылка на следующий hook)
- Цепочка функций: performWorkOnRoot => performUnitOfWork => beginWork => updateFunctionComponent => renderWithHooks => Component(App) => useState/useEffect => mount/update Reducer/Effect => mount/update WorkInProgressHook => ...
- renderWithHooks подключает настоящие hooks на время исполнения компонента. После исполнения компонента react их отключает настоящие hooks и ставит заглушки, которые выставляют ошибки. Этим реализуется идея, что нельзя вызывать hooks вне компонента
- логика поведения hooks подразделяется на mount/update фазы. Алгоритм определения mount или update указан ниже
```
ReactCurrentDispatcher.current = current === null || current . memoizedState === null 
```

State Hooks
- hook useState хранит финалььное рассчитанное состояние кадра в hook.memoizedState
- под капотом useState сведется к useReducer
- в своей работе опирается на дополнительные поля:
  - baseState
  - baseQueue
  - queue
- все новые изменения/update связанные с useState хранятся в hook.queue.pending
