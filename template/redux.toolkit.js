...
var createDraftSafeSelector = (...args) => {
  ...
  const wrappedSelector = (value, ...rest) => selector(isDraft(value) ? current(
    value) : value, ...rest);
...
var composeWithDevTools = typeof window
!== "undefined"
...
var devToolsEnhancer = typeof window
!== "undefined"
      ...
      measureUtils.measureTime(() => {
        ...
        result.wasMutated && invariant(
          ...
          || ""}. Take a look at the reducer(s) handling the action ${stringify(
            action)}. (https://redux.js.org/style-guide/style-guide#do-not-mutate-state)`
        );
      });
...
function createSerializableStateInvariantMiddleware(options = {}) {
  ...
  const {
    isSerializable = isPlain, getEntries, ignoredActions = [], ignoredActionPaths = [
      "meta.arg",
      "meta.baseQueryMeta"
    ], ignoredPaths = [], warnAfter = 32, ignoreState = false, ignoreActions = false, disableCache = false
  } = options;
  ...
  return (storeAPI) => (next) => (action) => {
    ...
    if (!ignoreActions && !(ignoredActions.length && ignoredActions.indexOf(
      action.type) !== -1)) {
      measureUtils.measureTime(() => {
...
function configureStore(options) {
  ...
  if (!IS_PRODUCTION && finalMiddleware.some((item) => typeof item
    !== "function")) {
    throw new Error(
...
function createReducer(
  ...
  let [actionsMap, finalActionMatchers, finalDefaultCaseReducer] = typeof mapOrBuilderCallback
  === "function"
  ...
  function reducer(state = getInitialState(), action) {
    let caseReducers = [
      ...
      ...finalActionMatchers.filter(({ matcher }) => matcher(action))
...
function createSlice(options) {
  ...
  function buildReducer() {
    ...
    const [extraReducers = {}, actionMatchers = [], defaultCaseReducer = void 0] = typeof options.extraReducers
    === "function"
      ? executeReducerBuilderCallback(options.extraReducers)
      : [options.extraReducers];
    const finalCaseReducers = __spreadValues(
...
function createUnsortedStateAdapter(selectId) {
  ...
  function updateManyMutably(updates, state) {
    ...
    if (didMutateEntities) {
      const didMutateIds = updates.filter((update) => takeNewKey(
        newKeys,
        update,
        state
      )).length > 0;
...
function createSortedStateAdapter(selectId, sort) {
  ...
  function addManyMutably(newEntities, state) {
    newEntities = ensureEntitiesArray(newEntities);
    const models = newEntities.filter((model) => !(selectIdValue(
      model,
      selectId
    ) in state.entities));
    if (models.length !== 0) {
    ...
    function actionCreator(arg) {
      return (dispatch, getState, extra) => {
        ...
        const promise2 = async function () {
          ...
          let finalAction;
          try {
            let conditionResult = (_a = options == null
              ? void 0
              : options.condition) == null ? void 0 : _a.call(
              options,
              arg,
              { getState, extra }
            );
            ...
            finalAction = err instanceof RejectWithValue ? rejected(
              null,
              requestId,
              arg,
              err.payload,
...
function hasExpectedRequestMetadata(action, validStatus) {
  ...
  const hasValidRequestStatus = validStatus.indexOf(action.meta.requestStatus)
    > -1;
...
function isAsyncThunkAction(...asyncThunks) {
  if (asyncThunks.length === 0) {
    return (action) => hasExpectedRequestMetadata(
      action,
...
var createFork = (parentAbortSignal) => {
  const linkControllers = (controller) => addAbortSignalListener(
    parentAbortSignal,
...
function createListenerMiddleware(middlewareOptions = {}) {
  ...
  const startListening = (options) => {
    let entry = findListenerEntry((existingEntry) => existingEntry.effect
      === options.effect);
    if (!entry) {
  ...
  const stopListening = (options) => {
    ...
    const entry = findListenerEntry((entry2) => {
      const matchPredicateOrType = typeof type === "string" ? entry2.type
  ...
  const notifyListener = async (entry, action, api, getOriginalState) => {
    ...
    try {
      ...
      await Promise.resolve(entry.effect(action, assign({}, api, {
        getOriginalState,
        condition: (predicate, timeout) => take(predicate, timeout)
          .then(Boolean),
        take,
...
var promise;
var queueMicrotaskShim = typeof queueMicrotask === "function"
  ? queueMicrotask.bind(typeof window !== "undefined" ? window : typeof global
  !== "undefined" ? global : globalThis)
  : (cb) => (promise || (promise = Promise.resolve())).then(cb)
